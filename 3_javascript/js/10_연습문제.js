...
for(let i=0 ; i<boxColor.length; i++){
    ...
    boxColor[i].addEventListener("change", function(){
        ...
        // box-color input 태그와 같은 인덱스 번째 box 요소 글씨색 변경
...
// #input에 작성된 값 만큼의 transition-duration을
// 모든 box 요소에 추가
// + #print1 요소의 내용을 #input1에 작성된 값을 변경

// 힌트.  transition-duration에 세팅되는 값은 초단위(s) 입니다.
...
document.getElementById("btn1").addEventListener("click", function(){
    ...
    for(let item of box){
        item.style.transitionDuration = ip1 + "s";
    }
