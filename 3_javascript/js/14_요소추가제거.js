...
// 안됨.  처음생성할때 remove라는 클래스를 가진 취소버튼이 없어서..
// 처음 HTML 문서 해석시 remove라는 클래스를 가진 요소가 존재하지 않기 때문에
// 아래 이벤트 동작 추가 구문이 아무 요소에서 적용되지 않음.
...
document.getElementById("calc").addEventListener("click", function(){
    ...
    // 함수는 호출된 시점의 HTML화면을 기반으로 해서 해석 진행!!
    ...
    for (let input of inputList){
        // sum += input.value; // input 요소에 작성된 값 ㅜ적
        //요소에서 읽어온 값 또는 내용은 무조건 String
        // -> 필요시 데이터 파싱 진행
        sum += Number(input.value); 
    }
    // const val = document.getElementsByClassName("in").value;
