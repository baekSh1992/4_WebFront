...
function getInnerHTML1(){
    ...
    // HTML 문서 내에서 id 속성값이 "test1"인 요소(Element)를 얻어와 
    // 상수형 변수 t1에 저장
    
    const t1 = document.getElementById("test1"); // 상수
    ...
    function getInnerText1(){
        // clg 자동 완성 -> console.log() (단, JS code ES6 snnipet 필요)
    ...
    // confirm
    function fnConfirm(){
        // console.log(window.confirm("테스트") ); // 웹에서 true / false 반환 확인

        // 조거문(if, switch) 작성법은 Java와 동일

        if( confirm("배경색을 분홍색으로 바꾸시겠습니까?") ){ // 확인 = 핑크
        ...
        // window.prompt("내용")
        function fnPrompt(){ // fn은 function의 약자 ( 변수명 할거 없을때 사용 )
